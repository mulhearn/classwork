...
For this assignment, we will use the wag-the-tail method to find the
energy eigenvalues of bound states of the 1-D Time-Independent
Schr\"odinger Equation (TISE):
\begin{equation}
- \frac{\hbar^2}{2 m} \; \ddpsidx \, + \, V(x) \, \psi(x) = E \, \psi(x)\\
\end{equation}
For bound states, the wave function diverges at $x=\pm\infty$ for
energies that are not energy eigenvalues.  It can be shown that the
sign of the divergence (to either positive or negative infinity)
changes at each energy eigenvalue.  Our approach is to find the roots
of
$$f(E) \equiv \psi_E(+\infty)$$ which are the energy eigenvalues.  We
will calculate $f(E)$ by numerically integrating the TISE out to a
...
so we should not see numbers like
$$\hbar = 1.054 \times 10^{-34} ~ \rm J\,s$$ appear\footnote{One
exception might be if we were writing a program that presents the
results of our calculation in SI units.  But even then, we would in
most cases calculate an appropriate scale factor that converts our
system of units into a reasonable SI unit at the correct scale for our
problem, avoiding factors like $10^{23} and 10^{-34}$.} in our
programs.  Instead, the results of our numerical calculations should
be arranged to be near one.  A good system of units removes tedious
calculations from our code, making it more elegant and easy to follow.
It makes it easy to spot bugs driving numbers very small or large.  It
often makes our numerical results more universal.  Just as
...
- \frac{\hbar^2}{2 m} \; \ddpsidx \, + \, V(x) \, \psi(x) = E \, \psi(x)\\
\end{equation}
We'll assume that our problem has some characteristic length scale $a$
and our particle has mass $m$.  We need not specify a particular value
of either quanity, and this is an example of how we can make our
numerical results more universal.

Multiplying both sides of the TISE by $a^2$ and rearranging we have:
\begin{equation}
...
While doing mathematical calculations, we preserve factors like $a$
and $E_0$.  This allow us to use dimensional analysis to check our
work, and to interpret our results in terms of SI units later if
necessary. In our programs, we set:
$$a = E_0 = 1$$ You can think of this as reporting our $x$ values in
units of $a$ (instead of meters) and energies in units of $E_0$
(instead of Joules).  This simplifies our code.  Computer variables
don't carry units, so we don't lose anything of value by dropping
these factors at the programming stage.  To be clear about this, $a$
and $E_0$ should therefore not appear anywhere in your program either
(they are 1).

\section{Conversion to First Order}

We will integrate this equation using the Runge-Kutta technique for a
first order differential equation, so we define a new variable $\phi$
by:
\begin{equation}
...
and
\begin{equation}
a \, \frac{dY}{dx} = \begin{pmatrix} {\displaystyle a \, \frac{d\psi}{dx}} \\[10pt] {\displaystyle a \, \frac{d\phi}{dx}} \end{pmatrix} = F(Y,x,E)
\end{equation}
where:
\begin{equation}
F(Y,x,E) \equiv \begin{pmatrix} \phi \\ \displaystyle \frac{(V(x)-E)}{E_0}\;\psi \end{pmatrix}
\end{equation}
for Euler's method, we approximate the change to $Y$ during a step in $x$ of size $h$ as:
$$K_1 = \frac{h}{a} a \frac{dY}{dx} = \frac{h}{a} F(Y, x, E)$$
...
\begin{eqnarray*}
K_1 &=& \frac{h}{a} \; \; F(Y, x, E) \\[5pt]
K_2 &=& \frac{h}{a} \; \; F\left(Y + \frac{K_1}{2}, x+\frac{h}{2}, E\right) \\[5pt]
K_3 &=& \frac{h}{a} \; \; F\left(Y + \frac{K_2}{2}, x+\frac{h}{2}, E\right) \\[5pt]
K_4 &=& \frac{h}{a} \; \; F(Y+K_3, x+h, E) \\
\end{eqnarray*}
where:
\begin{equation}
Y \equiv \begin{pmatrix} \psi \\ \phi \end{pmatrix}
...
\noindent
(A) Find the first six allowed energies of the harmonic oscillator potential using the wag-the-tail method.\\[5pt]
...
\noindent
(A) Find the first six allowed energies of the saw tooth potential using the wag-the-tail method.\\[5pt]
