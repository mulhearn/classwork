...
\title{Proposal for Revising the \\ Undergraduate Physics Curriculum \\ Version 1.10}
\author{Undergraduate Curriculum Committee}

\maketitle

\section{Changes Since the Faculty Meeting}
A number of changes have been made to the proposal, based on
discussion and feedback following the presentation of this proposal to
the faculty on June 3, 2020.  Those changes include:
\begin{itemize}
\item The new course PHY 115C is no longer a required course.
\item Symbolic Computation has been added to PHY 40.
\item Example syllabi for required courses impacted by this proposal are now included.
\item Proposed revisions to the Astrophysics and Applied Physics majors are now included.  
\item Some prerequisites were changed to accommodate the additional majors.
\end{itemize}
...
&or&&\\
\hline
PHY & 9HA & 5 & F & $\parallel$21B/21M & Honors Physics {\it (Class. Mech.)}\\ 
...
Course & & Units & Offered & Prereqs & Name \\
\hline
PHY & 104A & 4 & F & 9C/9HD,MAT 22B   & Mathematical Physics \\ 
...
PHY & 116A & 4 &  F & 80              & Phys. Instr. with A\&D Electronics.  \\
    & 116B & 4 &  W & 80              & Phys. Instr. for Data Acquisition. \\ 
\hline
    & or & & & & \\
\hline
...
\vskip 2cm
\captionof{table}{ An example schedule for an undergraduate physics major
    that takes the 9H series and 122A. Physics course numbers are
    shown with the number of units in parenthesis.  Math courses start
    with M.  Courses in italics are electives or have at least two
    different offerings per year. Course CAP is a capstone elective,
    and course X is any advanced elective.  Rate is 3-9 physics units
    per quarter in junior and senior year.}
...
\captionof{table}{ An example schedule for an undergraduate physics major
    that takes the 9 series and 122A. Physics course numbers are shown
    with the number of units in parenthesis.  Math courses start with
    M.  Courses in italics are electives or have at least two
    different offerings per year. Course CAP is a capstone elective,
    and course X is any advanced elective.  Rate is 7-13 physics units
    per quarter in junior and senior year.}
...
\vskip 1cm
\captionof{table}{An example schedule for an undergraduate physics
  major that transfers to UC Davis in their junior year, without
  Physics 9D or 40 equivalents, and takes 122A. Physics course numbers
  are shown with the number of units in parenthesis.  Courses in
  italics are electives or have at least two different offerings per
  year. Course CAP is a capstone elective, and course X is any
  advanced elective.  Rate is 12-13 units per quarter.  Note that only
  one upper division course is required in the first quarter of junior
  year, compared to three upper division courses in the current program.}
...
\item 110: The present curriculum devotes three quarters of required
  ...
  relativisitic mechanics, and introductory electricity and magnetism.
  This material must be covered adquately in the 9 series and 104A.
  The sequence will drop guided waves and radiation of moving charges,
  as well as simplify or skip less critical material.  Note that wave
  guides are an example topic for 110L.
...
\item New computational physics courses 45, 110L, 112L, and 115L
  ...
  required four-unit course students are required to take at least two
  of the three one-unit computing lab courses. This is discussed in
  detail in Section~\ref{sec:comp}.

\item The 116ABC sequence adds PHY 80 as a prerequisite and is
  reduced to a two quarter sequence 116AB.  Due to the significant
  overlap between 116A+116C and 80, no material is lost.  This is
...
increases requires an additional instructor (currently typically two,
although three were initially planned for 2020-2021) there would be a
decrease of 0.3 instructors.  Additional fall offerings for 122A/B
could be added as well, if needed to meet demand.
...
\end{itemize}
The entire Scientific Python  ecosystem is easily accessible to students for any
major OS for free through Anaconda.  No graphical features of C/C++
will be explored.  If necessary, C++ programs will pass data by text
file to SciPy for plotting.  SymPy is not as mature as Mathematica,
but comes with the significant advantages of being freely available
within Anaconda and interoperable with SciPy.

In response to the ECS department's decision to no longer offer ECS
30, a one quarter introduction to C without any prerequisites, we
replaced this required course with own version, PHY 40, in 2018. This
course currently introduces programming with both C++ and python.
This leaves no time for symbolic computation and little time for
exploring additional features of Scientific Python beyond the Python
language.  The proposal keeps 40 as a physics-centric introduction to
programing and adds a second, more advanced computational course, PHY
45, with both 40 and the 9 series as prerequisites.  The proposal
adds upper division computing labs as well.  The computational courses
added or impacted by this proposal are:

\begin{itemize}
\item 40: This four-unit course has no prerequisites and assumes no
  prior knowledge of programing.  It is primarily an introduction to
  programming but using examples from computational physics.  It will
  also include a short introduction to symbolic manipulations using a
...
\item 80: is a new four-unit lab course introduced in 2018 which
  ...
  In this proposal, 80 is required for all majors and adds 40 as a
  prerequisite.
...
\item 110L, 112L, and 115L: These three new one-unit computational lab
  ...
  statistical mechanics, and quantum mechanics.  The emphasis will be on
  solving problems from upper division physics using computing at the
  level of PHY 45.  The instructor will present a technique and
  problem during a single one hour lecture, and the students will have
  the rest of the week to complete the assignment.  One instructor
  could teach all three of these courses in a single year, which will
  count as one course.  Each course is offered in a different quarter,
  so students get three units of computational problem-solving spread
  across an entire year.  Due to unit limits, we only require two of
  these courses, but all three are recommended.
...
offered in fall and winter respectively.
...
% top tier:
\node[right] at (0,6.5) {122AB 115B {\underline{115C}}, Capstone Courses};
...
It appears from the graph that 45 is a bottleneck as well, but it is
less severe.  It does not require MAT 22A so can be taken sooner than
104A, and it is only needed for top tier courses such as 115C and the
computational lab courses, which can be postponed until senior year.
...
\item The Physical and Chemical Oceanography course (GEL 150A) is not offered every year and now has extensive prerequisites.  Therefore, the Physical Oceanography major might not be achievable in two years for students that have not already satisfied these prerequisites.  The prerequisites for reaching GEL 150A are now included in the concentration courses, and GEL 150B has been added.  See Table~\ref{tbl:oceanography}.
...
{\bf Example schedules:} In most cases, the 9D requirement is
...
\vskip 0.25cm
{\bf Concentration Courses:  }\\
Units:  37 units. *: recommended, $^\#$: not offered every year, $\parallel$: concurrently.\\
\begin{tabular}{|llllll|}
\hline
Course & & Units & Offered & Prereqs & Name \\
\hline
...
{\bf Example schedules:} for the junior and senior year.  In most
cases, the PHY 9D and ENG 17 requirement is satisfied before the
...
\newpage
\captionof{table}{\label{tbl:materials}Materials Science}
\noindent
\vskip 0.25cm
{\bf Concentration Courses:  }\\
Units:  37-38 units. *: recommended, $^\#$: not offered every year, $\parallel$: concurrently.\\
\begin{tabular}{|llllll|}
\hline
Course & & Units & Offered & Prereqs & Name \\
\hline
...
\vskip 0.25cm
{\bf Concentration Courses:  }\\
Units:  37-40 units. *: recommended, $^\#$: not offered every year, $\parallel$: concurrently.\\
\begin{tabular}{|llllll|}
\hline
Course & & Units & Offered & Prereqs & Name \\
\hline
...
ATM & 60     & 4 & F   & & Intro. to Atmospheric Sci. \\
    & 120    & 4 & F   & & Atmos. Thermo. and Cloud Physics \\
...
PHY  & 45     & 4 &     & & \\
GEL  & 50     & 3 & FWS & & Physical Geology \\
     & 55     & 3 & F   & & Intro. to Geochemistry\\
     & 116N   & 3 & S      & GEL 50          & Oceanography\\
     & 150A   & 4 & S$^\#$ & GEL 116N,55 & Physical and Chemical Oceanography\\
     & 150B   & 3 & W      & GEL 50          & Geological Oceanography\\
\hline
    & Choose two of & & & & \\
...
\end{tabular}\\ \vskip 0.25cm
\noindent
{\bf Total Units:} 108-109
{\bf Note: } This degree may not be possible to complete in two years.
Students should seek instructor permission to take GEL 150A alongside
prerequisites if it is offered in their junior year.\\
{\bf Example schedule:} In most cases, the PHY 9D and ENG 45 requirement is
satisfied before the junior year, but an example including these courses is
shown.  Physics course numbers are shown unless otherwise indicated.
Courses in italics are electives.
...
Junior    & 40         & 45           & 80 \\
          ...
          & (9D)       & GEL 50       & GEL 159N \\
\hline
Senior   & GEL 55      & GEL 150B         & GEL 150A \\
         & 112+L         & 140A       & 140B \\
         & 115A          & 115B       & 122A \\
         &               & 116B       &  \\
\hline
...
\vskip 0.25cm
{\bf Concentration Courses:  }\\
Units:  36 units. *: recommended, $^\#$: not offered every year, $\parallel$: concurrently.\\
\begin{tabular}{|llllll|}
\hline
Course & & Units & Offered & Prereqs & Name \\
\hline
...
allow some flexibility to adjust the course content purely within the
department without contradicting the course catalog.

Instructors are expected to satisfy the brief course description as listed
in the course catalog, but are not obligated to follow the detailed
example syllabi shown here.  However, many of these courses are
prerequisite courses, so instructors are expected to consider the
impact on downstream courses to their decisions.
...
Example coverage:
\begin{itemize}
\item Quick Review of Complex Numbers (Ch. 2)
\item Quick Review of Sequences and Series, including Taylor (1.1-1.15)
\item Quick Review of Linear Transformations (3.6-3.12)
\item Review of Vector Calculus (Ch. 6) (as needed)
\item Fourier Series (7.1-7.11)
\item Fourier Transform (7.12, 13.9)
\item Dirac Delta Function ( 8.11)
\item Legendre Series (12.1-12.10)
\item Partial Differential Equations (13.2)
\item Series Solutions to PDEs (13.3-13.8)
\item Calculus of Variations (Chap 9) (if time permits)
\end{itemize}
\end{samepage}
...
This course, which has one hour of lecture per week, includes extensive problems from numerical physics related to PHY 110AB, using the techniques introduced in PHY 40 and 45.
...
\item Waveguides (Recommended Topic... not covered in 110AB)
\end{itemize}
...
\vskip 1cm
\noindent
{\bf PHY 116A --- Physics Instrumentation with Analog and Digital Electronics:}  
...
\vskip 1cm
\noindent 
{\bf PHY 116B --- Physics Instrumentation for Data Acquisition:}  
...
\vskip 1cm
\noindent {{\bf PHY 110C}—Electricity \& Magnetism (4):}  
Theory of electrostatics, electromagnetism, Maxwell's equations, electromagnetic waves.

\vskip 1cm
\noindent {{\bf PHY 112}—Thermodynamics \& Statistical Mechanics (4):}  
Theory of electrostatics, electromagnetism, Maxwell's equations, electromagnetic waves.
Introduction to classical and quantum statistical mechanics and their connections with thermodynamics. Theory is developed for the ideal gas model and simple magnetic models and then extended to studies of solids, quantum fluids, and chemical equilibria. GE credit: SE. Effective: 2000 Winter Quarter.
