...
%\vskip 1cm
%{\bf Problem 7:} (Optional and Challenging) If we define an
%experimental estimate of a quantity $f(x)$ from $N$ measured values
%$\{x_i\}$ as:
%\begin{displaymath}
%\bar{f} = \frac{1}{N}\sum_i f(x_i)
%\end{displaymath}
%(a) Show that:
%\begin{displaymath}
%  \braket{\bar{f}} = \braket{f(x)}
%\end{displaymath}
%using the fact that each $x_i$ is presumed to be drawn from $P(x)$ and so 
%\begin{displaymath}
%  \braket{f(x_i)} = \braket{f(x)}.
%\end{displaymath}
%(b) Next, assume that:
%\begin{displaymath}
%  \braket{f(x)} = 0
%\end{displaymath}
%just to keep the math easier, and show that:
%\begin{displaymath}
%  \braket{\bar{f}^2} = \braket{f^2(x)} / N. 
%\end{displaymath}
%Start by writing:
%\begin{displaymath}
%  \bar{f}^2 = \left( \frac{1}{N} \sum_i f(x_i)\right) \, \left( \frac{1}{N} \sum_j f(x_j)\right) 
%\end{displaymath}
%and take the expectation value of both sides.  Note that for
%$i \neq j$ then $x_i$ and $x_j$ are independent random variables and so
%\begin{displaymath}
%  \braket{f(x_i) f(x_j)} =  
%  \begin{cases}
%    0, & i \neq j \\   
