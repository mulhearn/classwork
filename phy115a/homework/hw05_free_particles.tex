...
\noindent
(B) Use Fourier's trick to determine the formulas for the coefficients $c_m$.  That is, calculate:
...
\noindent
(A) Find $\hat{p}^\dagger$ by writing $\braket{\hat{p} f | g}$ as an integral, and doing whatever it takes to move the action over to $g$.

\noindent
(B) Show that this definition this is equivalent to definition used by Griffiths:
$$\braket{f|\hat{O} g} = \braket{\hat{O}^\dagger f | g}$$

\noindent
(C) Show that $\left(\hat{O}^\dagger\right)^\dagger = \hat{O}$

\noindent
(D) Show that $\left(\hat{A}\hat{B}\right)^\dagger = \hat{B}^\dagger\hat{A}^\dagger$

\noindent
(E) Is $\left(i\hat{x}\right)$ a Hermetian operator?

\noindent
(F) Is $\left(\hat{x}\hat{p}\right)$ a Hermetian operator?

\noindent
(G) Is $\left(\frac{\partial^2}{\partial x^2}\right)$ a Hermetian operator?
...
$\braket{\hat{a}_-\hat{a}_+}$ in similar fashion to determine $|d_n|^2$.
...
$$d_n = \sqrt{n+1}$$
Hint: calculate $$\braket{\hat{a}_+\hat{a}_-}$$ a third way.

\noindent
(E) Suppose we picked instead:
$$c_n = i\sqrt{n}$$
what would $d_n$ be?  Would the spatial wave functions $\psi_n(x)$ be real functions?
