...
First off, note that the periodic complex-valued functions with period $a$ form a vector space just as well as periodic real functions.  Next, note that a periodic complex-valued functions $\psi(x)$ has a Fourier series, as we can just write:
$$\psi(x) = f(x) + i g(x)$$ 
...
In this section, we explicitly work out the Fourier Series for Complex-Valued Functions from Euler's Identities and the Fourier Series for Sines and Cosines.
