...
Now for the bad news.  The classical free particle has velocity
...
But that problem seems minor compared to the next one.  We can calculate:
...
We'll need some more mathematical tools to tackle these problems...
...
And that explains the problems.  $\sin^2(\theta)$ is never negative, because $\sin(\theta)$ is a real valued function.  But $(e^i\theta)^2$ can be negative, because $e^i\theta$ is a complex number. For example, $e^{i\pi/2} = i$, so $\left( e^{i\pi/2} \right)^2 = -1$.  With this understood, we know exactly what we have to do, we want the integral to be:
\begin{equation}
\label{eqn:normexpfs}
...
\begin{eqnarray*}
f(x) &=& \sum_{m=-\infty}^{\infty}  c_m \, e_m(x) \\[5pt]
\braket{e_n|f} &=& \int_{-\frac{a}{2}}^{\frac{a}{2}} e_n^*(x) \sum_{m=-\infty}^{\infty}  c_m \, e_m(x) dx\\[5pt]
 &=& \sum_{m=-\infty}^{\infty}  c_m \int_{-\frac{a}{2}}^{\frac{a}{2}} e_n^*(x) \, e_m(x) dx\\[5pt]
...
\section{Resolution of the Problems with the Free Particle}

We are now ready to solve the problem with our Free Particle stationary state solutions.  We just have to accept that the simple plane wave solutions are not physical solutions, but they do still provide a complete solution to the SE for the free particle.  Let's see how it works.

We are given a wave function at $\Psi(x,0)$ at $t=0$ describing the initial state.  This function must be normalized.  We calculate it's Fourier transform:
...
The beautiful thing is, we know the time dependence for each traveling wave $e^{ikx}$ that appears in the integral.  These are stationary solution the TISE, so they have a wiggle factor:
