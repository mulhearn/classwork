...
But:
\begin{eqnarray*}
A \sin(k_{-n}x) = A \sin(-k_{n}x) = (-A) \sin(k_{n}x)
\end{eqnarray*}
and since $A$ is just a constant, we can absorb the minus sign, and so we can omit negative values of $n$.  Furthermore,
\begin{eqnarray*}
A \sin(k_{0}x) = A \sin(0) = 0
\end{eqnarray*}
which is (again) not normalizable, so we can omit $n=0$.  

We are left at last with an infinite number of solutions:
\begin{equation}
\psi_n(x) = \sin(k_n a)
\end{equation}
where
...
k_n&=&\frac{n\pi}{a}
\end{eqnarray*}
and $n$ is a quantum number with $n=1,2,3,\ldots$.
