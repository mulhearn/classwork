...
\end{displaymath}
where $\lambda$ is the wavelength of the light, $d$ the distance between the slit, $\theta$ the angle relative to the center of the slits, and $I_0$ the intensity at $\theta=0$.  You will derive this result in the homework. 
...
\item JJ Thomson discovered the electron (1897) using cathode ray tubes and determined it's charge to mass ratio, Michelson oil dop.  More recently, bubble chambers 

\end{itemize}
Despite these impressive achievements, a series of experiments revealed the shortcomings of classical physics:
\begin{itemize}
...
\end{itemize}
These results made it clear that classical physics needed to be replaced or overhauled.
The conversation between experimentalists and theorists during this time was quite intense.  There were both breakthroughs and missteps, but ultimately, the theory of Quantum Mechanics emerged as an even greater accomplishment than the Classical Physics it superseded.

This is a fascinating period to read about, and there is much to be learned by looking closely at past breakthroughs and missteps.  However, our approach will be to jump directly to our present understanding of Quantum Theory, learn how to apply it, develop intuitions about it, and then see how the canonical historical experiments are all described handily by the theory.

\section{An Experimental Perspective}

\begin{itemize}

\item Never forget that physics is an {\bf experimental discipline}: the
most compelling mathematical argument you can imagine could be rendered
invalid by a single well-executed experiment.
...
alot by seeing how different assumptions lead to the same physical
theory.  We'll do some of this.

\item You can appreciate the beauty of an elegant derivation or the
power of a well-designed notation while still keeping the experimental
perspective!
...
\item A particle is described by a wave function $\Psi(x,t)$.

\item The time evolution of $\Psi(x,t)$ according to the Schroginger Equation (SE):
\begin{equation}
\label{eqn:se}
