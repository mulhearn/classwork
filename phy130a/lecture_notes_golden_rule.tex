...
$$\ket{i} = \ket{p_a \, p_b \, \ldots} = \ket{p_a} \times \ket{p_b} \times \ldots$$
and the final states is:
$$\ket{f} = \ket{p_1 \, p_1 \, \ldots} = \ket{p_1} \times \ket{p_1} \times \ldots$$
...
\begin{equation}
 \label{eqn:subm}  
 |T_{fi}|^2 \to \frac{|M_{fi}|^2}{\left( \prod 2\,E_a \right) \left( \prod 2\,E_i \right)}
\end{equation}
where $M_{fi}$ is the new Lorentz-invariant matrix element that will replace $T_{fi}$ in our calculations.  Applying Equations~\ref{eqn:nrphin} and \ref{eqn:subm} to Fermi's Golden Rule as written in Equation~\ref{eqn:nrfgr}, to obtain:
\begin{equation}
\label{eqn:covfgr}
\Gamma_{fi} \; = \; \frac{1}{\hbar \prod (2 E_a)} \; \int |\mathcal{M}_{fi}|^2 \; (2\pi)^4 \delta^4(\sum p_a - \sum p_1) 
...
case as least, we see that FGR is a Lorentz-covariant equation.
...
$$\vec{p}_2 = -\vec{p}_1$$
so that:
$$\Gamma_{fi} \; = \; \frac{1}{32 \pi^2 m_a \hbar} \; \int |\mathcal{M}_{fi}|^2 \; \delta^4(E_1 + E_2 - m_a)
\, \frac{d^3p_1}{E_1 E_2}$$
Defining:
$$p \equiv |\vec{p}_1|$$
...
we have
$$\Gamma_{fi} \; = \; \frac{1}{32 \pi^2 m_a \hbar} \; \int |\mathcal{M}_{fi}|^2 \; \delta^4(u - m_a)
...
We now have:
$$\Gamma_{fi} \; = \; \frac{1}{32 \pi^2 m_a \hbar} \; \int |\mathcal{M}_{fi}|^2 \; \delta^4(u - m_a) \, \frac{p}{u}\, du d\Omega$$
...
time, as it must be, to be logically conistent with indentical
...
observable in experients, for example, by looking at the distance
...
takes.  Griffith's evades the topic entirely by just giving FGR for
...
Our calculation of FGR is also modified, with the density of states
taking on a factor of volume (see the Appendix for a concrete
example):
$$\sum_{\Delta f} \to \int_{\Delta f} \, dn = V \int_{\Delta f} \,
\rho(E_f) \, dE_f$$
So our presciption for modifying the NR FGR becomes:
\begin{equation}
\label{eqn:voltfi}
|T_{fi}|^2 \to \frac{V |M_{fi}|^2}{(\prod 2 V E_a) (\prod 2 V E_i) }  
\end{equation}
and
\begin{equation}
\label{eqn:volps}
(2\pi) \delta(E_f - E_i) dn \to (2 \pi)^4 \delta^4(\sum p_i - \sum p_a) \prod_i \frac{V d^3 p_i}{(2\pi)^3}
\end{equation}
...
\noindent
{\bf Exercise:} Show that Equation \ref{eqn:fgr-two-body-decay} is unchanged when using the substitutions in Equations~\ref{eqn:voltfi} and \ref{eqn:volps}, which explictly including the volume factors,instead of Equations~\ref{eqn:nrphin} and \ref{eqn:subm}.
...
$$a + b \to 1 + 2 + 3 + \ldots$$
as:
$$\Gamma_{fi} \; = \; \frac{1}{V} \, \frac{1}{\hbar 2 E_a 2 E_b} \;
...
$$\sigma \; = \; \frac{1}{4 E_a E_b (v_a + v_b)} \, \frac{1}{\hbar}\;
\int |\mathcal{M}_{fi}|^2 \; (2\pi)^4 \delta^4(\sum p_a - \sum p_1)
\prod_i \frac{d^3p_i}{(2 E_i) \, (2\pi)^3 }$$
...
$$\sigma \; = \; \frac{1}{4 \sqrt{(p_a\cdot p_b)^2 - (m_a m_b)^2}} \, \frac{1}{\hbar}\;
\int |\mathcal{M}_{fi}|^2 \; (2\pi)^4 \delta^4(\sum p_a - \sum p_1)
\prod_i \frac{d^3p_i}{(2 E_i) \, (2\pi)^3 }$$
...
The interaction cross-sections is bread-and-butter experimental and
...
Equation~\ref{eqn:volps}, but for a factor of $\hbar$, a matter of
convention.  Note that in the convention here, $dn$ is dimensionless,
consistent with our framing of the problem as counting discrete states
of a particle constrained to a finite volume $V$.
